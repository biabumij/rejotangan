...
class Rap extends Secure_Controller {
	...
	public function form_bahan()
	{
		...
		if ($check == true) {
			$data['products'] = $this->db->select('*')->order_by('nama_produk','asc')->get_where('produk', array('status' => 'PUBLISH', 'kategori_produk' => 1))->result_array();
	...
	public function submit_rap_bahan()
	{
		...
		$penawaran_boulder = $this->input->post('penawaran_boulder');
		...
		$arr_insert = array(
			...
			'measure_a' => $measure_a,			
			'produk_a' => $produk_a,
			'penawaran_boulder' => $penawaran_boulder,
			'price_a' => $price_a,
