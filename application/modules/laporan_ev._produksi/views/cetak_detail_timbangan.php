...
<html>
	...
	<body>
		<div align="center" style="display: block;font-weight: bold;font-size: 11px;">Timbangan</div>
		...
		<table width="98%" border="0" cellpadding="3" border="0">
			...
			$timbangan_biaya_all = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			...
			$timbangan_jurnal_all = $this->db->select('sum(pdb.debit) as total')
			...
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			$total_nilai_timbangan_all = $timbangan_biaya_all['total'] + $timbangan_jurnal_all['total'];
			$total_nilai_evaluasi_timbangan_all = ($total_nilai_timbangan_all!=0)?$timbangan - $total_nilai_timbangan_all * 1:0;

			$timbangan_biaya = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			->join('pmm_detail_biaya pdb','pb.id = pdb.biaya_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (233)")
			->where("pb.status = 'PAID'")
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			...
			$penyusutan_timbangan_biaya = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			->join('pmm_detail_biaya pdb','pb.id = pdb.biaya_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (234)")
			->where("pb.status = 'PAID'")
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
	
			$penyusutan_timbangan_jurnal = $this->db->select('sum(pdb.debit) as total')
			->from('pmm_jurnal_umum pb ')
			->join('pmm_detail_jurnal pdb','pb.id = pdb.jurnal_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (234)")
			...
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			$total_nilai_penyusutan_timbangan = $penyusutan_timbangan_biaya['total'] + $penyusutan_timbangan_jurnal['total'];
			?>
			...
			<tr class="table-baris1" style="font-weight:bold;">		
				<th align="left" style="border-left:1px solid black;">Timbangan</th>
				...
				<th align="right" style="border-right:1px solid black;"><?php echo number_format($total_nilai_timbangan_all,0,',','.');?></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<?php
				$styleColor = $total_nilai_evaluasi_timbangan_all < 0 ? 'color:red' : 'color:black';
				?>
				<th align="right" style="<?php echo $styleColor ?>; border-right:1px solid black;"><?php echo $total_nilai_evaluasi_timbangan_all < 0 ? "(".number_format(-$total_nilai_evaluasi_timbangan_all,0,',','.').")" : number_format($total_nilai_evaluasi_timbangan_all,0,',','.');?></th>
	        </tr>
			<tr class="table-baris1">			
				<th align="left" style="border-left:1px solid black;">&nbsp;&nbsp;Biaya Alat Timbangan</th>
				<th align="center" style="border-right:1px solid black;">M3</th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"=><?php echo number_format(0,0,',','.');?></th>
				...
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><a target="_blank" href="<?= base_url("laporan/cetak_detail_biaya_alat_timbangan?filter_date=".$filter_date = date('d-m-Y',strtotime($date1)).' - '.date('d-m-Y',strtotime($date2))) ?>"><?php echo number_format($total_nilai_timbangan,0,',','.');?></a></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
	        </tr>
			<tr class="table-baris1">			
				<th align="left" style="border-left:1px solid black; border-bottom:1px solid black;">&nbsp;&nbsp;Penyusutan Timbangan</th>
				...
				<th align="right" style="border-bottom:1px solid black;"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-bottom:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black; border-bottom:1px solid black;"><a target="_blank" href="<?= base_url("laporan/cetak_detail_penyusutan_timbangan?filter_date=".$filter_date = date('d-m-Y',strtotime($date1)).' - '.date('d-m-Y',strtotime($date2))) ?>"><?php echo number_format($total_nilai_penyusutan_timbangan,0,',','.');?></a></th>
				<th align="right" style="border-bottom:1px solid black;"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-right:1px solid black; border-bottom:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
	        </tr>
	    </table>
