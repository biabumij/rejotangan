...
<html>
	...
	<body>
		<div align="center" style="display: block;font-weight: bold;font-size: 11px;">Biaya Stone Crusher + Genset</div>
		...
		<table width="98%" border="0" cellpadding="3" border="0">
			...
			$stone_crusher_biaya_all = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			...
			$stone_crusher_jurnal_all = $this->db->select('sum(pdb.debit) as total')
			...
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			$total_nilai_stone_crusher_all = $stone_crusher_biaya_all['total'] + $stone_crusher_jurnal_all['total'];
			$total_nilai_evaluasi_stone_crusher_all = ($total_nilai_stone_crusher_all!=0)?$stone_crusher - $total_nilai_stone_crusher_all * 1:0;

			$stone_crusher_biaya = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			->join('pmm_detail_biaya pdb','pb.id = pdb.biaya_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (225)")
			->where("pb.status = 'PAID'")
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			...
			$penyusutan_stone_crusher_biaya = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			->join('pmm_detail_biaya pdb','pb.id = pdb.biaya_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (226)")
			->where("pb.status = 'PAID'")
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
	
			$penyusutan_stone_crusher_jurnal = $this->db->select('sum(pdb.debit) as total')
			->from('pmm_jurnal_umum pb ')
			->join('pmm_detail_jurnal pdb','pb.id = pdb.jurnal_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (226)")
			...
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			$total_nilai_penyusutan_stone_crusher = $penyusutan_stone_crusher_biaya['total'] + $penyusutan_stone_crusher_jurnal['total'];

			$genset_biaya = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			->join('pmm_detail_biaya pdb','pb.id = pdb.biaya_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (228)")
			->where("pb.status = 'PAID'")
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			...
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			$total_nilai_genset = $genset_biaya['total'] + $genset_jurnal['total'];

			$penyusutan_genset_biaya = $this->db->select('sum(pdb.jumlah) as total')
			->from('pmm_biaya pb ')
			->join('pmm_detail_biaya pdb','pb.id = pdb.biaya_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (229)")
			->where("pb.status = 'PAID'")
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
	
			$penyusutan_genset_jurnal = $this->db->select('sum(pdb.debit) as total')
			->from('pmm_jurnal_umum pb ')
			->join('pmm_detail_jurnal pdb','pb.id = pdb.jurnal_id','left')
			->join('pmm_coa c','pdb.akun = c.id','left')
			->where("pdb.akun in (229)")
			...
			->where("(pb.tanggal_transaksi between '$date1' and '$date2')")
			->get()->row_array();
			$total_nilai_penyusutan_genset = $penyusutan_genset_biaya['total'] + $penyusutan_genset_jurnal['total'];
			...
			<tr class="table-baris1" style="font-weight:bold;">		
				<th align="left" style="border-left:1px solid black;">Stone Crusher + Genset</th>
				...
				<th align="right" style="border-right:1px solid black;"><?php echo number_format($total_nilai_stone_crusher_all,0,',','.');?></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<?php
				$styleColor = $total_nilai_evaluasi_stone_crusher_all < 0 ? 'color:red' : 'color:black';
				?>
				<th align="right" style="<?php echo $styleColor ?>; border-right:1px solid black;"><?php echo $total_nilai_evaluasi_stone_crusher_all < 0 ? "(".number_format(-$total_nilai_evaluasi_stone_crusher_all,0,',','.').")" : number_format($total_nilai_evaluasi_stone_crusher_all,0,',','.');?></th>
	        </tr>
			<tr class="table-baris1">			
				<th align="left" style="border-left:1px solid black;">&nbsp;&nbsp;Biaya Alat Stone Crusher</th>
				<th align="center" style="border-right:1px solid black;">M3</th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"=><?php echo number_format(0,0,',','.');?></th>
				...
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><a target="_blank" href="<?= base_url("laporan/cetak_detail_biaya_alat_sc?filter_date=".$filter_date = date('d-m-Y',strtotime($date1)).' - '.date('d-m-Y',strtotime($date2))) ?>"><?php echo number_format($total_nilai_stone_crusher,0,',','.');?></a></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
	        </tr>
			<tr class="table-baris1">			
				<th align="left" style="border-left:1px solid black;">&nbsp;&nbsp;Penyusutan Stone Crusher</th>
				<th align="center" style="border-right:1px solid black;">M3</th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"=><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><a target="_blank" href="<?= base_url("laporan/cetak_detail_penyusutan_sc?filter_date=".$filter_date = date('d-m-Y',strtotime($date1)).' - '.date('d-m-Y',strtotime($date2))) ?>"><?php echo number_format($total_nilai_penyusutan_stone_crusher,0,',','.');?></a></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
	        </tr>
			<tr class="table-baris1">			
				<th align="left" style="border-left:1px solid black;">&nbsp;&nbsp;Biaya Alat Genset</th>
				<th align="center" style="border-right:1px solid black;">M3</th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"=><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right"><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><a target="_blank" href="<?= base_url("laporan/cetak_detail_biaya_alat_genset?filter_date=".$filter_date = date('d-m-Y',strtotime($date1)).' - '.date('d-m-Y',strtotime($date2))) ?>"><?php echo number_format($total_nilai_genset,0,',','.');?></a></th>
				<th align="right"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-right:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
	        </tr>
			<tr class="table-baris1">			
				<th align="left" style="border-left:1px solid black; border-bottom:1px solid black;">&nbsp;&nbsp;Penyusutan Genset</th>
				...
				<th align="right" style="border-bottom:1px solid black;"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-bottom:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
				<th align="right" style="border-right:1px solid black; border-bottom:1px solid black;"><a target="_blank" href="<?= base_url("laporan/cetak_detail_penyusutan_genset?filter_date=".$filter_date = date('d-m-Y',strtotime($date1)).' - '.date('d-m-Y',strtotime($date2))) ?>"><?php echo number_format($total_nilai_penyusutan_genset,0,',','.');?></a></th>
				<th align="right" style="border-bottom:1px solid black;"><?php echo number_format(0,2,',','.');?></th>
				<th align="right" style="border-right:1px solid black; border-bottom:1px solid black;"><?php echo number_format(0,0,',','.');?></th>
	        </tr>
	    </table>
